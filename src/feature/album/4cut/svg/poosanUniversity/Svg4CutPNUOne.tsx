...
import {
  PNU_ONE_DEFS,
  PNU_ONE_INNER_AFTER_PHOTOS,
} from './Svg4CutPNUOneStatic';

/** viewBox 1080×1920 기준, Svg4Cut과 동일: TL → TR → BR → BL */
const PHOTO_SLOTS = [
  { x: 65, y: 149 },
  { x: 552, y: 78 },
  { x: 552, y: 789 },
...
export interface Svg4CutPNUOneProps {
  width: number | `${number}`;
...
export default function Svg4CutPNUOne({
  photos,
  height,
  width,
}: Svg4CutPNUOneProps) {
  const reactId = useId().replace(/:/g, '');
  const clipId = `pnu1-clip-${reactId}`;
  const patternId = `pnu1-pattern-${reactId}`;
  const imageId = `pnu1-qr-${reactId}`;

  const photoSources = photos ?? [];
  const ids = { clipId, patternId, imageId };

  const innerHtml = applyPnuTemplateIds(PNU_ONE_INNER_AFTER_PHOTOS, ids);
  const defsHtml = applyPnuTemplateIds(PNU_ONE_DEFS, ids);
